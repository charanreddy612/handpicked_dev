...
export async function list({
  ...
  let query = supabase.from("coupons").select(selectCols).order("created_at", { ascending: false }).range(from, to);
  ...
  // Count
  let countQuery = supabase.from("coupons").select("id", { count: "exact", head: true });
...
export async function togglePublish(id) {
  const { data: cur, error: ge } = await supabase.from("coupons").select("is_publish").eq("id", id).single();
  if (ge) throw ge;
  const next = !cur?.is_publish;
  const { data, error } = await supabase.from("coupons").update({ is_publish: next }).eq("id", id).select().single();
  if (error) throw error;
  return data;
}

export async function toggleEditorPick(id) {
  const { data: cur, error: ge } = await supabase.from("coupons").select("is_editor").eq("id", id).single();
  if (ge) throw ge;
  const next = !cur?.is_editor;
  const { data, error } = await supabase.from("coupons").update({ is_editor: next }).eq("id", id).select().single();
  if (error) throw error;
  return data;
}
